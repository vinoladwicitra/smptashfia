  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const { data, error } = await supabase
        .from('articles')
        .select(`id, title, slug, excerpt, featured_image, published_at, article_category_mappings (article_categories (name, slug))`)
        .eq('status', 'published')
        .order('published_at', { ascending: false })
        .limit(5);
      if (!error && data) {
        setPosts(data.map((a: any) => ({ ...a, category: a.article_category_mappings?.[0]?.article_categories?.name || 'Umum', date: formatDate(a.published_at) })));
      }
      setLoading(false);
    };
    fetchPosts();
          ...
          <div className="hidden lg:grid lg:grid-cols-4 lg:gap-4 lg:mb-10">
            <div onClick={() => { window.scrollTo({ top: 0, behavior: 'smooth' }); navigate(`/blog/${display[0].slug}`); }} className="col-span-2 row-span-2 relative rounded-xl overflow-hidden shadow-md group cursor-pointer aspect-square">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5">
                ...
                <div className="flex items-center gap-3 text-xs text-white/70 mb-3"><span className="flex items-center gap-1"><IconCalendar size={13} />{display[0].date}</span></div>
                <div className="flex items-center gap-1.5 text-white/80 text-sm font-medium group-hover:text-white transition-colors">Baca Selengkapnya <IconArrowRight size={14} /></div>
              </div>
            </div>
            {display.slice(1).map((post: any) => (
              <div key={post.id} onClick={() => { window.scrollTo({ top: 0, behavior: 'smooth' }); navigate(`/blog/${post.slug}`); }} className="relative rounded-xl overflow-hidden shadow-md group cursor-pointer aspect-square">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4">
                  ...
                  <div className="flex items-center gap-1.5 text-white/80 text-xs font-medium group-hover:text-white transition-colors">Baca <IconArrowRight size={12} /></div>
                </div>
              </div>
            ))}
          </div>
            ...
            {display.map((post: any) => (
              <div key={post.id} onClick={() => { window.scrollTo({ top: 0, behavior: 'smooth' }); navigate(`/blog/${post.slug}`); }} className="flex h-28 bg-white rounded-xl overflow-hidden shadow-sm active:scale-[0.98] transition-transform cursor-pointer">
                ...
                <div className="flex-1 h-28 p-2.5 flex flex-col overflow-hidden">
                  <div>
                    <div className="mb-1"><span className={`px-1.5 py-0.5 text-[10px] font-semibold rounded ${categoryStyles[post.category] || 'bg-gray-100 text-gray-700'}`}>{post.category}</span></div>
                  ...
                  <div className="flex items-center justify-between mt-auto pt-1"><span className="flex items-center gap-1 text-[9px] text-text-light"><IconCalendar size={10} />{post.date}</span></div>
                </div>
              </div>
            ))}
          </div>
