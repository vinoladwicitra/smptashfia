...
export default function PublicBlogSingle() {
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      setLoading(true);
      const { data: articleData, error } = await supabase.from('articles')
  ...
  if (loading) {
    return <div className="min-h-screen bg-background flex items-center justify-center"><svg className="animate-spin h-8 w-8 text-primary" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg></div>;
  }
  ...
  return (
    <main className="min-h-screen bg-background pb-32 lg:pb-0">
      ...
      <div className="sm:hidden fixed bottom-[76px] left-1/2 -translate-x-1/2 z-[900]">
        <div className="flex items-center gap-0 bg-white/95 backdrop-blur-sm border border-black/[0.03] rounded-full shadow-lg px-2 py-1.5" style={{ boxShadow: '0 10px 40px rgba(0,0,0,0.12)' }}>
          {/* Like */}
          <button onClick={handleLike} className={`flex items-center gap-1 px-1.5 py-1 rounded-full transition-colors ${liked ? 'text-rose-600' : 'text-text-light hover:text-rose-600'}`}>
            <div className={`w-[34px] h-[34px] flex items-center justify-center rounded-full ${liked ? 'bg-rose-50' : 'bg-gray-100'}`}>{liked ? <IconHeart size={16} className="fill-rose-500 text-rose-500" /> : <IconHeart size={16} />}</div>
            <span className="text-xs font-medium ml-1">{likeCount}</span>
          </button>

          {/* Bookmark */}
          <button onClick={handleBookmark} className={`flex items-center px-1.5 py-1 rounded-full transition-colors ${saved ? 'text-primary' : 'text-text-light hover:text-text'}`}>
            <div className={`w-[34px] h-[34px] flex items-center justify-center rounded-full ${saved ? 'bg-primary/10' : 'bg-gray-100'}`}>{saved ? <IconBookmarkFilled size={16} /> : <IconBookmark size={16} />}</div>
          </button>
          ...
          <button onClick={() => { if (!inArticle || readProgress === 0) window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="flex items-center px-1.5 py-1 rounded-full text-text-light transition-colors cursor-pointer">
            <div className={`w-[34px] h-[34px] flex items-center justify-center rounded-full ${inArticle ? 'bg-gray-100' : 'bg-primary/10 text-primary'}`}>
              {inArticle && readProgress > 0 ? <span className="text-xs font-medium text-text">{readProgress}%</span> : <IconArrowUp size={16} />}
            </div>
          </button>
        </div>
      </div>
          ...
          <div className="bg-white rounded-t-2xl w-full p-6 animate-slideUp" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-4 gap-4">
              ...
              <button onClick={() => handleShare('copy')} className="flex flex-col items-center gap-2">
                <div className="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center"><IconCopy size={24} className="text-text" /></div>
